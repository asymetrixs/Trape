...
namespace trape.cli.trader.Analyze
{
    ...
    /// </summary>
    public class Analyst1h3h : IAnalyst
    {
        #region Fields
        ...
        public Analyst1h3h(ILogger logger, IBuffer buffer, IAccountant accountant)
        {
            ...
            this._logger = logger.ForContext<Analyst1h3h>();
            this._buffer = buffer;
        ...
        private async Task _recommend(string symbol)
        {
            ...
            // MA3h * 1.008 (0.8%) is higher than MA1h AND Slope1h and Slope1h are positive
            var distanceOkAndTrendPositive = (stat10m.MovingAverage1h * 1.008M) > stat10m.MovingAverage1h
                                                && stat10m.Slope30m > -0.0065M && stat10m.Slope1h > 0.009M;
            ...
            this._logger.Debug($"S10m:{stat2m.Slope10m:0.0000} S15m:{stat2m.Slope15m:0.0000} S30m:{stat10m.Slope30m:0.0000} S1h:{stat10m.Slope30m:0.0000} MA30m:{stat10m.MovingAverage30m:0.00} MA1h:{stat10m.MovingAverage1h:0.00}");
            ...
            if (stat3s.Slope5s < -2M
                ...
                && panicLimitMA1h < stat10m.MovingAverage1h
            ...
            // Trend negative
            else if (stat10m.Slope1h < 0)
            {
                // MA short below long
                if (stat10m.MovingAverage30m < stat10m.MovingAverage1h)
                {
                    if (stat10m.Slope30m < -0.015M)
                    {
                        this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m < -0.015M");
                    ...
                    {
                        this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m < 0");

                        action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat10m.Slope30m > 0.015M && currentPrice < stat10m.MovingAverage1h)
                    {
                        this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m > 0.015M && currentPrice < MovingAverage1h");
                    ...
                    else if (stat10m.Slope30m > 0 && currentPrice < stat10m.MovingAverage1h)
                    {
                        this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m > 0 && currentPrice < MovingAverage1h");
                    ...
                    {
                        if (stat2m.Slope15m < 0)
                        {
                            this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m > 0 && currentPrice > MovingAverage1h; Slope15m < 0");

                            action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                        }
                        else
                        {
                            this._logger.Verbose("Slope1h < 0; MovingAverage30m < MovingAverage1h; Slope30m > 0 && currentPrice > MovingAverage1h; [else]");
                ...
                {
                    if (stat10m.Slope30m > 0 && stat2m.Slope15m < 0)
                    {
                        this._logger.Verbose("Slope1h < 0; MovingAverage30m >= MovingAverage1h; Slope30m > 0 && Slope15m < 0");
            ...
            else if (stat10m.Slope1h > 0)
            {
                ...
                if (stat10m.MovingAverage30m < stat10m.MovingAverage1h)
                {
                    if (stat10m.Slope30m > 0.01M && stat2m.Slope15m > 0)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m > 0.01M && Slope15m > 0");

                        action = this.StrongBuyStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h, distanceOkAndTrendPositive);
                    }
                    else if (stat10m.Slope30m > -0.002M && stat2m.Slope15m > 0)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m > -0.002M && Slope15m > 0");

                        action = this.NormalBuyStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h, distanceOkAndTrendPositive);
                    }
                    else if (stat10m.Slope30m > -0.008M && stat10m.Slope30m <= -0.002M)
                    {
                        if (stat2m.Slope15m > 0)
                        {
                            this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m > -0.008M && Slope30m <= -0.002M; Slope15m > 0");

                            action = datalayer.Enums.Action.Hold;
                        }
                        else
                        {
                            this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m > -0.008M && Slope30m <= -0.002M; [else]");

                            action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                        }
                    }
                    else if (stat10m.Slope30m > -0.025M && stat10m.Slope30m <= -0.008M)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m > -0.025M && Slope30m <= -0.008M");

                        action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat10m.Slope30m <= -0.025M)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; Slope30m <= -0.025M");

                        action = this.StrongSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m < MovingAverage1h; [else]");
                ...
                {
                    if (stat2m.Slope15m < 0 && stat10m.Slope30m < 0)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m >= MovingAverage1h; Slope15m < 0 && Slope30m < 0");

                        action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat10m.Slope30m < -0.01M)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m >= MovingAverage1h; Slope30m < -0.01M");

                        action = this.StrongSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat2m.Slope10m > 0 && stat2m.Slope15m > 0 && stat10m.Slope30m > -0.005M)
                    {
                        this._logger.Verbose("Slope1h > 0; MovingAverage30m >= MovingAverage1h; Slope10mm > 0 && Slope15m > 0 && Slope30m > -0.005M");
            ...
            this._logger.Debug($"Decision - Calculated / Final: {calcAction} / {action}");
        ...
        public datalayer.Enums.Action NormalSellStrategy(Stats3s stat3s, Stats15s stat15s, Stats2m stat2m, Stats10m stat10m, Stats2h stat2h, decimal lowerLimitMA1h, decimal upperLimitMA1h)
        {
            ...
            return datalayer.Enums.Action.Sell;
        }
        ...
        public datalayer.Enums.Action NormalBuyStrategy(Stats3s stat3s, Stats15s stat15s, Stats2m stat2m, Stats10m stat10m, Stats2h stat2h, decimal lowerLimitMA1h, decimal upperLimitMA1h, bool distanceOkAndTrendPositive)
        {
            ...
            return datalayer.Enums.Action.Buy;
        }
        ...
        public datalayer.Enums.Action StrongBuyStrategy(Stats3s stat3s, Stats15s stat15s, Stats2m stat2m, Stats10m stat10m, Stats2h stat2h, decimal lowerLimitMA1h, decimal upperLimitMA1h, bool distanceOkAndTrendPositive)
        {
            ...
            return datalayer.Enums.Action.StrongBuy;
        }
