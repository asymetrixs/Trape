...
namespace trape.cli.trader.Analyze
{
    ...
    /// </summary>
    public class Analyst30m1h : IAnalyst
    {
        #region Fields
        ...
        public Analyst30m1h(ILogger logger, IBuffer buffer, IAccountant accountant)
        {
        ...
        private async Task _recommend(string symbol)
        {
            ...
            decimal upperLimitMA1h = stat10m.MovingAverage3h * 1.0002M;
            decimal lowerLimitMA1h = stat10m.MovingAverage3h * 0.9998M;
            decimal panicLimitMA1h = stat10m.MovingAverage3h * 0.9975M;

            // MA3h * 1.008 (0.8%) is higher than MA1h AND Slope3h and Slope3h are positive
            var distanceOkAndTrendPositive = (stat10m.MovingAverage3h * 1.008M) > stat10m.MovingAverage3h
            ...
            this._logger.Debug($"S10m:{stat2m.Slope10m:0.0000} S15m:{stat2m.Slope15m:0.0000} S30m:{stat10m.Slope1h:0.0000} S1h:{stat10m.Slope1h:0.0000} MA30m:{stat10m.MovingAverage1h:0.00} MA1h:{stat10m.MovingAverage3h:0.00}");
            ...
            // Trend negative
            else if (stat10m.Slope3h < 0)
            {
                // MA short below long
                if (stat10m.MovingAverage1h < stat10m.MovingAverage3h)
                {
                    if (stat10m.Slope1h < -0.015M)
                    {
                        this._logger.Verbose("Slope3h < 0; MovingAverage1h < MovingAverage3h; Slope1h < -0.015M");
                    ...
                    {
                        this._logger.Verbose("Slope3h < 0; MovingAverage1h < MovingAverage3h; Slope1h < 0");

                        action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat10m.Slope1h > 0.015M && currentPrice < stat10m.MovingAverage3h)
                    {
                        this._logger.Verbose("Slope3h < 0; MovingAverage1h < MovingAverage3h; Slope1h > 0.015M && currentPrice < MovingAverage3h");
                    ...
                    {
                        this._logger.Verbose("Slope3h < 0; MovingAverage1h < MovingAverage3h; Slope1h > 0 && currentPrice < MovingAverage3h");
                    ...
                    else if (stat10m.Slope1h > 0 && currentPrice > stat10m.MovingAverage3h)
                    {
                        ...
                        {
                            this._logger.Verbose("Slope3h < 0; MovingAverage1h < MovingAverage3h; Slope1h > 0 && currentPrice > MovingAverage3h; [else]");
            ...
            else if (stat10m.Slope3h > 0)
            {
                ...
                if (stat10m.MovingAverage1h < stat10m.MovingAverage3h)
                {
                    if (stat10m.Slope1h > 0.01M && stat10m.Slope30m > 0)
                    {
                        this._logger.Verbose("Slope3h > 0; MovingAverage1h < MovingAverage3h; Slope1h > 0.01M && Slope30m > 0");
                    ...
                    else if (stat10m.Slope1h > -0.008M && stat10m.Slope1h <= -0.002M)
                    {
                        ...
                        {
                            this._logger.Verbose("Slope3h > 0; MovingAverage1h < MovingAverage3h; Slope1h > -0.008M && Slope1h <= -0.002M; [else]");

                            action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                        }
                    }
                    else if (stat10m.Slope1h > -0.025M && stat10m.Slope1h <= -0.008M)
                    {
                        this._logger.Verbose("Slope3h > 0; MovingAverage1h < MovingAverage3h; Slope1h > -0.025M && Slope1h <= -0.008M");

                        action = this.NormalSellStrategy(stat3s, stat15s, stat2m, stat10m, stat2h, lowerLimitMA1h, upperLimitMA1h);
                    }
                    else if (stat10m.Slope1h <= -0.025M)
                    {
                        this._logger.Verbose("Slope3h > 0; MovingAverage1h < MovingAverage3h; Slope1h <= -0.025M");
                ...
                else
                {
                    ...
                    {
                        this._logger.Verbose("Slope3h > 0; MovingAverage1h >= MovingAverage3h; Slope1h < -0.01M");
        ...
        private void _logTrend(Recommendation recommendation, Stats10m stat10m, Stats2h stat2Hours)
        {
            ...
            if (DateTime.UtcNow.Second != this._logTrendLimiter)
            {
                ...
                this._logger.Verbose($"{recommendation.Symbol}: {reco} | S1h: {stat10m.Slope3h:0.0000} | S2h: {stat10m.Slope2h:0.0000} | MA1h: {stat10m.MovingAverage3h:0.0000} | MA2h: {stat10m.MovingAverage2h:0.0000} | MA6h: {stat2Hours.MovingAverage6h:0.0000}");
            }
        }
